...
//imagetalkl[mai_nutoral]{
ブラックボックステストはその名の通りテスト対象を挙動の分からない「ブラックボックス」として扱うことから来たテストだね。変な値を試して失敗するか、正しい値を入れて成功するか等を見ていくね。
//}
...
//imagetalkl[mai_nutoral]{
来てほしくないデータが入ってきたら、エラーを出す、というところまでがプログラムに期待されるからね。エラーにすべき入力で正しいエラーを得る、これも重要なテスト項目というわけだ。
//}

//imagetalkr[akina_ooo]{
なるほど、エラーにすべき値というものは一般的に判明していたりするんですか？
//}

//imagetalkl[mai_nutoral]{
シンプルな括りだと異常値テストや境界値テストっていうものがあるね。これもそれぞれ見ていこう。
//}

==== 異常値テスト

//imagetalkl[mai_nutoral]{
異常値テストという分類ではプログラムが本来受け取らないはずの大きな数値や型の異なるものを入力として受けた際にエラーとして返すかどうかを考えるね。実は入力を受け付けてしまって予期しないループ構文で恐ろしいほどループしてしまっていた、なんてこともあるね。
//}
...
//imagetalkl[mai_nutoral]{
あとは「境界値テスト」だね。入力時に予期している（ｘ～ｙ）の値の範囲があると、そのx-1やy+1など正常に処理される値の一つ隣の値で予期した形の失敗になるかどうかを確認するね。
//}

//imagetalkr[akina_nutoral]{
なるほど、内部条件文のボーダーの内側と外側でちゃんと結果が変わっていることを確認するんですね。重要そうです。
//}

//imagetalkl[mai_niko]{
ＯＫ！いいね！次からはもう少しややこしいから気を引き締めてかかろう。
//}

==== ホワイトボックステスト

//imagetalkl[mai_nutoral]{
「ホワイトボックステスト」はこれまでと打って変わってプログラムの中身の挙動がどうなるかを分かったうえで行うテストだ。基本的には条件分岐項目に注目して、すべてのパターンの値を見ていこう、というテストだね。
//}

//imagetalkr[akina_ooo]{
なるほど、すべてを見るのは大変そうですが、重要な気がします。
//}

//imagetalkl[mai_nutoral]{
そこで考案されたのがテストカバレッジレベルだ。テストケースの幅の広さなどに関してC0,C1,C2,MC/DC などのレベルがあるね。一個ずつ見ていこう
//}

==== 命令網羅 -C0

//imagetalkl[mai_nutoral]{
まずは命令網羅(C0)だ。これはプログラム内に存在するコードに対して必ず一回はテストを行うこと、という定義だ。条件分岐内に専用のコードがある場合にそのコードを実行するテストであればOKだね。
//}

//imagetalkr[akina_ooo]{
分岐内の専用のコードですか・・・ということは[A=B]って条件でのみ動くコードがある時に、[A=B]の条件をクリアして実行していればいいってことですか？
//}
...
==== 分岐網羅-C1
//imagetalkl[mai_nutoral]{
分岐網羅(C1)はすべての分岐を最低一回ずつ網羅するものだね。[A=B]って条件で動くコードがある時に、[A=B]のと[A≠B]のケースを実行するテストがあればOKだ。
//}

//imagetalkr[akina_nutoral]{
なるほど、分岐先を網羅するっていうのはイメージしやすいですね。
//}

==== 条件網羅

//imagetalkl[mai_nutoral]{
条件網羅は条件式の結果のそれぞれを網羅することになる。判断結果ではなく条件が正しく動いていることを確認するテストだ。[A=BもしくはC=D]という条件があった場合に[A=B]、[A≠B]、[C=D]、[C≠D]の４つの条件を実行できればいい。言ってしまえば[A=BかつC≠D]と[A≠BかつC=D]の2パターンを実行してしまえばこの条件をクリアできるわけだ。
//}

//imagetalkr[akina_nutoral]{
ちょっと複雑になってきましたね。条件式のパーツのパターンを網羅するって感じですかね。
//}
...
//imagetalkl[mai_nutoral]{
条件判断網羅は条件式パターンの網羅に加えて判断結果も網羅する。判断結果のtrue/falseを網羅できて、[A=B]、[A≠B]、[C=D]、[C≠D]の４つの条件を実行できればいい。
//}

//imagetalkr[akina_komari]{
んん？あ、そうか！さっきの条件網羅は[A=BもしくはC=D]という条件に対して[A=BかつC≠D]と[A≠BかつC=D]の2パターンだったから判断結果がどちらもtureだったのか！
//}

//imagetalkl[mai_niko]{
そうそう、条件網羅ではカバーできない範囲を確認するために考えられたのがこの考えだね。でもじつは[A=BかつC=D]と[A≠BかつC≠D]を実行すればいいんだけどね。
//}

//imagetalkr[akina_ettu]{
え？結局二つなんですか？でも確かに判断結果はtrueとfalseに分かれてるし、条件も４つそれぞれ実行してるのか。
//}

//imagetalkl[mai_nutoral]{
まあ条件文が複雑になればなるほどパターンは指数関数的に増えるからね。３つネストされたってだけでテストコードも煩雑になる。今は思考実験としてシンプルにしてるけど実際の作業量は相当多いね。
//}

//imagetalkr[akina_hee]{
なるほど、今のところ違いは小さく見えますが現場ではこの違いが大きく影響するんですね。
//}

==== 改良条件判断網羅- MC/DC

//imagetalkl[mai_nutoral]{
この改良条件判断網羅(MC/DC)は条件判断網羅に加えて、条件の各パーツが単独で判断結果影響するものを追加でテストする。
//}
...
//imagetalkl[mai_nutoral]{
だよねぇ。まぁ[A=BもしくはC=D]って条件の場合、A=BかC=Dのどっちかがtureであれば判断結果はtureだよね？
//}
...
//imagetalkl[mai_nutoral]{
片方がfalseでも、もう片方がtrueなら結果がtrueになる。つまりもう片方が単独で判断結果に影響していると言えるよね？
//}
...
//imagetalkl[mai_nutoral]{
ということで、片方がfalseでもう片方がtureで判断結果がtrueになるパターンと両方falseで判断結果がfalseになるパターンが作れればいい。
//}
...
//imagetalkl[mai_nutoral]{
片方がtrueの時点でもう片方は単独で判断結果に影響を及ぼさないから両方trueは必要ないって結論になるね。
//}

//imagetalkr[akina_ooo]{
なるほど。でもこの考え方って必要なんですか？
//}

//imagetalkl[mai_niko]{
実は結構重要。なんでかっていうと、単独で判断結果に影響しない条件へのテストはしなくていいけどある程度の品質は保証されるからだ。
//}

//imagetalkr[akina_hee]{
なるほど！判断結果に強く影響しない条件文があるとしたらケースの大幅な削減になりますね！
//}

==== 複合条件カバレッジ（C2）

//imagetalkl[mai_nutoral]{
あらゆる条件式のパターンを網羅する考え方が複合条件カバレッジだね。直感的にもこれだとテストコードが爆発的に増えやすいのがわかるね。実際に注目したい機能以外へのテストコードが多すぎるから、このC2が現場で使われるのはあんまり見てないね。
//}

//imagetalkr[akina_ooo]{
そうなんですか。まいさんはどんなものを使ってたんですか？
//}

//imagetalkl[mai_nutoral]{
C0,C1,MC/DCあたりかな。単体テストならC0,C1でもいいんだけど内部結合テストになるとMC/DCとかが複雑になってくるからね。限られたリソースなら一旦はこれぐらいが落としどころだと思うよ。
//}

//imagetalkr[akina_nutoral]{
なるほど。このテスト方式は内部結合のAPIテストまで持っていくんですか？
//}

//imagetalkl[mai_nutoral]{
APIテストに行くまでに内部結合テストで直行表やデシジョンテーブルを使った複数クラスのテストを行うことがおおいね。
//}
...
//imagetalkl[mai_niko]{
デシジョンテーブルは内部結合テスト以降で関連のあるクラスや判断条件をまとめた行列の図表を作成して、テスト実装の優先順位をつけるものだね。関連するクラスが多くなればなるほどこの作業がとても重要になる。
//}

//imagetalkr[akina_hee]{
なるほど、複雑な条件のロジックとかをテストする際に情報整理になりそうですね！
//}

==== リソース状況に適したテスト

//imagetalkl[mai_nutoral]{
ここまででテストの深さの分類を見てきたけど、APIテストにまで行っちゃえばAPIの中にはデータベースからデータを取ってくるだけのものもあったりして全部が全部ホワイトボックステストの方式に合わなかったりする。多くの場面で、APIテストはブラックボックステストを行うことが多いね。
//}

//imagetalkr[akina_nutoral]{
なるほど。ホワイトボックスが必要なかったり、逆に複雑すぎることもあるんですね。
//}

//imagetalkl[mai_nutoral]{
ちなみに、単体テストレベルまでならプログラムコードから自動で解析してC0,C1などのテストコードを生成してくれるツールも存在する。
//}
...
//imagetalkl[mai_nutoral]{
無料のものからちょっとお高いものまでいろいろあるよ。ただ、プロジェクト規模が大きく人件費がかかってしまうプロジェクトなら人を減らして有料のテスト生成ツールの導入に踏み切るケースもあるね。柔軟に対応していきたいものだ。
//}

//imagetalkr[akina_nutoral]{
そうですね。私も最適のものが選べるよう頑張ります！
//}
...
ISBN-13  :  978-4296103577

おすすめポイント: 実際に陥りがちなテストのアンチパターンを紹介しつつ、その原因となる開発現場のジレンマの問題分析と解決策の提示を行っている良書。実装例はないが、テスト管理者およびテスト設計者にとって助けとなるテスト計画のための視点が凝縮されている。実際の品質を向上するための有効なテスト計画について心強い助言が多く書かれている。
...
//imagetalkl[mai_nutoral]{
次は移植性の高いプログラムを作ろうという観点だ。
//}

//imagetalkr[akina_nutoral]{
移植性って、再利用のしやすさとかでしたっけ？
//}

//imagetalkl[mai_nutoral]{
再利用もそうだし検証用などの開発環境にそれぞれ挙動を確認しやすくすることをイメージしてもらってもいいね。
//}

//imagetalkr[akina_nutoral]{
なるほど、環境再現は重要になるシーンがあるんですもんね！
//}
...
=== OS/ミドルウェアに依存しないプログラム

//imagetalkl[mai_nutoral]{
OS/ミドルウェアに依存しないプログラム言語でWEB系に使われやすいのはPythonやJavaScript、PHP、Ruby等があるね。
//}
...
//imagetalkl[mai_nutoral]{
OSに依存しないって言うのはそうだけど、最初からWEB系に向けた開発に向いてるわけじゃなくてTomcatやほかのフレームワークに依存する部分が大きいからミドルウェア依存が強いっていう点で先に挙げた4種類とは勝手が違うね。
//}
...
//imagetalkl[mai_nutoral]{
C#やSwiftなどのネイティブアプリ系言語とかはそもそも成果物が動くOSが固定されていたりするから移植性をあまり意識されていない設計と捉えることができる。その一方で開発環境を含めて専用のリッチなコンテンツ開発に目を向けてるともいえるね。
//}

//imagetalkr[akina_nutoral]{
なるほど、自分の場所で勝負する方向のものもあるんですね。
//}

//imagetalkl[mai_nutoral]{
その点、この移植性を気にするのはWEBプログラミング特有の問題なのかもしれないね。まぁ、とりあえず現在の人気言語であればOS/ミドルウェア依存は大きくない設計だから、可能な限りそれらの制約の少ない言語を選ぼう。
//}
...
//imagetalkl[mai_nutoral]{
次にネットワークに依存しないプログラムだ。
//}

//imagetalkr[akina_ettu]{
ネットワークに依存しないって、どういう意味ですか？ネットワークが無ければ通信ができないと思いますが。
//}

//imagetalkl[mai_nutoral]{
たしかにこれは定義が甘いかもね。厳密に言えば「特定ネットワーク内でしか稼働できないプログラムを避けよう」という話だ。特にあるのが開発用ライブラリが特定ネットワークの中からしかアクセスできなくて、簡単に検証用環境を立てられなかったりする問題があるね。
//}

//imagetalkr[akina_ooo]{
なるほど、たしかに状況によって開発すらできないのは厳しいですね。
//}


//imagetalkl[mai_nutoral]{
もちろんワークプレースセキュリティ設定によってプロジェクト資産の持ち出しが制限されていると、現地にまで行って開発を行わなきゃいけないし、検証用PCへのデータの移動に大仰な申請が必要だったりするからね。大変だ。
//}
...
//imagetalkl[mai_nutoral]{
結構実在するケースなんだよね。私としてはこの問題を解決するだけでどれだけ生産性があがるかを定性的・定量的に評価できればお客さんを納得させることができると思うけどね。
//}

//imagetalkr[akina_hee]{
あ、でもコロナ禍でクラウド移行し始めてるお客さんも多いって聞きますし、この先はそうなっていくんじゃないですかね？
//}
...
//imagetalkl[mai_nutoral]{
次はAPIベース設計についてだ。これはビジネスロジックなどの機能的な処理の呼び出し口を、個々にWeb APIのような形で切り出していく設計のことだね。
//}

//imagetalkr[akina_nutoral]{
Web APIってHTTPとかでアクセスするサービスとかが多いイメージですけど、それがお客さんの環境内の単体サービス開発に必要なんですか？
//}

//imagetalkl[mai_nutoral]{
必要かって言われると微妙かもしれないけど、努力目標であり恩恵が多岐にわたる優れた設計であるのは確かだね。
//}

//imagetalkr[akina_nutoral]{
恩恵が多い・・・ですか。どんなものがあるんですか？
//}
...
//imagetalkl[mai_nutoral]{
APIベースの設計をすることで、テストの設計がしやすくなる。入力と出力の形が規定されて入力のパターンが明確になるし、どういうケースでエラーになるべきかが外部設計や要件定義の成果物と照らし合わせやすくなるんだ。
//}

//imagetalkr[akina_hee]{
なるほど、APIというまとまりが設計時の定義に対比しやすい大きさになるんですね！
//}

//imagetalkl[mai_nutoral]{
多くの場合にそうだね。テストケースを考えやすいしテストコードもAPIを実行する形にするだけで複雑な前処理とかも大幅に減る。これがWEB APIになってなくて特定のクラスをロードしてテストコードに組み込まないといけないことと比べれば負担の軽さは明白だし、テストコードを書くことに熟練していない人でもコードが書きやすいんだ。
//}

//imagetalkr[akina_hee]{
	クラスのインスタンス化や初期化を行ったりする必要がなくなるなら考えることが少なくて済みそうですね！
//}

==== モックによる並行開発性の向上

//imagetalkl[mai_nutoral]{
次は「モックによる並行開発性の向上」だ。APIとしての設計をすることで、それを使う側はモックを作成してあてがうことでAPIが出来上がっていなくても自分の開発を続けることができる。
//}
...
//imagetalkl[mai_nutoral]{
そうだね。スタブに似てるけど大体は単純なオブジェクトだったり動的な機能のないクラスだったりするね。APIを実行したら帰ってくるはずの形のものを真似れていればいいから、設計さえできてれば実質作れたも同然だね。
//}

//imagetalkr[akina_ooo]{
設計さえできていれば作れたも同然ってすごい言葉ですね。あれ、もしかして設計ができていればモックが作れて、さらにテストコードも書けたりしませんか？
//}


//imagetalkl[mai_niko]{
鋭いね！設計が成功ケースや失敗ケースまで考慮しているものであればテストコードも作れる。完全に機能ごとに分業できる上にテストに失敗していないかをコードを作る側と使う側の両方で監視しあえるんだ。テストケースが足りない場合も指摘しあえるし、お互いに支えあうための良い設計になると思わない？
//}

//imagetalkr[akina_nutoral]{
喧嘩にならなきゃいいですけど・・・プロジェクトの終盤で燃えるよりも開発途中で相談できるなら儲けものですね。
//}

==== 画面とロジックの分離性の向上

//imagetalkl[mai_nutoral]{
さっきはモックによって分業が可能になるって言ったけど、これは「画面とロジックの分離性の向上」にもつながるね。画面側をつくるフロントエンジニアもAPIができる前からモックを利用して画面のデータを用意することができるし、ほしいデータが足りない時にどのバックエンドエンジニアに相談すればいいかも分かりやすい。
//}

//imagetalkr[akina_hee]{
なるほど、画面側ももらえるデータの形がわかるのは作業がはかどりますね！
//}

==== サービスの再利用性の向上

//imagetalkl[mai_nutoral]{
次は「サービス再利用性の向上」だ。同時に環境再現性の向上も含んでいるね。仮想化等によってサービスAPIが稼働する仮想マシンを作ることができれば普段のネットワーク外でも開発サーバーを立てることで環境を再現して開発・テストを行うことができる。
//}
...
//imagetalkl[mai_nutoral]{
もう少し後でも紹介するけど自分のOS内に別のOSが独立して稼働しているかのように見せかける仮想化技術の一つだね。使いたいAPIサーバーの仮想マシンをローカルに立てれば自分のローカル環境でもテストが行えるようになる。
//}

//imagetalkr[akina_hee]{
多くのサーバーとつないだ環境ではじめてできていたテストが楽になるんですね！
//}

//imagetalkl[mai_niko]{
そうそう！APIサーバーに問題があればAPIサーバー開発チームに相談すればいいし、特定ネットワークからのデータ取得が必要だったとしても、仮想マシン側に保存した状態で再配布できるから、継続的な展開もしやすいね。
//}
...
//imagetalkl[mai_niko]{
ガンガン使おう！これからもっと大事になるからね！
//}

=== 洗練されたプログラミングパターン

//imagetalkl[mai_nutoral]{
ここまででプログラムの保守性、省力性、信頼性、移植性の4つの観点に触れてきたけどこれらの観点以外にも「良いプログラムをどう作ればよいか」を突き詰めた結果提唱されてきた考え方がいくつかある。「デザインパターン」や「ドメイン駆動」などは日本の開発現場でも比較的取り入れられているシーンが多いね。
//}

//imagetalkr[akina_ooo]{
「ドメイン駆動」というのは分かりませんが、「デザインパターン」は前のプロジェクトで一部を学びながら実装しました。
//}
...
//imagetalkl[mai_niko]{
すばらしい！「デザインパターン」は開発現場で見かけることの多いプログラム構造を抽象化して23のパターンにまとめたものだ。これらのパターンを開発者同士が理解していることで複雑な設計に関しても齟齬が発生することなく設計方針を固められる、共通言語のようなものだと思ってもらっていいね。私の主観ではあるけれど、Factory MethodパターンやFacadeパターンなどは特に現場でよく見かけるね。
//}

//imagetalkr[akina_hee]{
それです！Factory Methodパターンでした。先輩たちがそういう話をしていた気がします。
//}

//imagetalkl[mai_nutoral]{
いいね！「デザインパターン」に関してはネットにも詳しい資料があるし書籍も沢山出てるから勉強しやすいと思うよ。
//}

//imagetalkr[akina_nutoral]{
前のプロジェクトでは使うものしか勉強しなかったので、他のパターンもしっかり勉強してみます。
//}
...
おすすめポイント:デザインパターンを学ぶ際の決定版ともいえる名著。各デザインパターンをJava言語のシンプルな実装例とともに紹介している。パターンごとに知識確認のための練習問題もあり、理解を深める助けになっている。文量が多く、読み解くには体力が必要だが読みぬくことができればプログラマとして確実に実力を上げていける一冊
...
//imagetalkl[mai_nutoral]{
「ドメイン駆動」はオブジェクト指向プログラミングの発展形の一つで、業務領域のデータの取り扱い方に重点を置いた設計の手法だね。業務の各シーンにおいてに必要な情報の集まりをオブジェクトとしてモデリングして、オブジェクトを利用する振る舞いを表すサービスというクラスを通して操作する形にまとめるものだね。プロジェクト参加者間でこの設計思想への理解がある場合、この設計思想に合わせて開発することでスムーズな開発を行うことができる。
//}

//imagetalkr[akina_ooo]{
開発者間の共通認識という点では「デザインパターン」と似た立ち位置なんですかね？
//}

//imagetalkl[mai_nutoral]{
そうだね。「ドメイン駆動」は「デザインパターン」のうちのいくつかのパターンと相性がいいことも指摘されているけど、より具体的な設計思想だと思ってもらった方がいいね。
//}
...
//imagetalkl[mai_nutoral]{
比較的って話だけどね。あと、業務領域の知識をオブジェクトに落とし込む、という過程は抽象化の作業が多いしいろんな手法が提案されているから「モデリング」やモデリング結果を表す図表の形式の「UML」という観点でも調べてみるのもいいかもね。
//}
...
//image[domaindriven][][scale=0.5]

ISBN-10  :  479815072X

ISBN-13  :  978-4798150727

出版社  :  翔泳社; 第1版 (2020/2/13)

発売日  :  2020/2/13

おすすめポイント:ドメイン駆動設計に関して諸学者に分かりやすい説明に重点を置きつつ体系的にまとめている一冊。全編をC#のサンプルコードで説明しているがオブジェクト指向言語への知識があれば問題なく理解できる。各章ごとに関連する章を明記しており、各要素が互いに意味を与えあうドメイン駆動の設計思想の理解の助けになる。
