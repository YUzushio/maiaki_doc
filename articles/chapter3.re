...
//imagetalkl[mai_niko]{
まずはリモートリポジトリからローカルリポジトリにコードをダウンロードするプルの話だ。
//}

//imagetalkr[akina_nutoral]{
単純に引っ張ってくるってイメージであってますか。
//}

//imagetalkl[mai_nutoral]{
うん、そうだね。リモートブランチの特定ブランチからローカルリポジトリのブランチにコードをダウンロードできる。ローカルブランチ側で別の名前にすることもできるね。
//}

//imagetalkr[akina_nutoral]{
なるほど、手元でも名前を変えて管理できるならコードのダウンロードも気軽にできますね。
//}


//imagetalkl[mai_nutoral]{
そう、実際に同じ名前のブランチで問題が発生してしまっても別のブランチ名でプルしてマージすることもできる。安全だし便利だね。
//}


=== プッシュ
//imagetalkl[mai_nutoral]{
	次はローカルリポジトリからリモートリポジトリにコードをアップロードするプッシュの話だ。
//}

//imagetalkr[akina_nutoral]{
プルと対比して押し上げるってイメージですね。
//}

//imagetalkl[mai_nutoral]{
そうだね。やることは単純にアップロードなんだけど、このプッシュに関してはリモートリポジトリのコミットをローカルリポジトリでプルしている必要がある。リモートリポジトリよりもローカルリポジトリのコミットが進行していなければプッシュできない使用になってるんだ。
//}

//imagetalkr[akina_nutoral]{
なるほど、古いものはプッシュできないんですね。
//}


//imagetalkl[mai_nutoral]{
自分が開発していてリモートリポジトリに誰かがプッシュしてしまった時も、自分の作業をローカルでコミットしてからプルをして、最後にプッシュする必要があるね。
//}
...
//imagetalkl[mai_niko]{
安全にコードを更新するためだからね。これぐらいは必要コストとして覚えよう。
//}
...
//imagetalkl[mai_nutoral]{
プルとプッシュを繰り返してコードが成長していくと最終的なコードの確定にリモートリポジトリのマージ操作「プルリクエスト」が発生するんだ。
//}
...
//imagetalkl[mai_nutoral]{
基本的に動きは同じなんだけど重要性の違いから別の表現で表されるね。多くのリモートリポジトリではプルリクエストに関しては専用の画面を用意していてどのブランチからどのブランチにどんなコードが送られようとしているかを確認できる。
//}

//imagetalkr[akina_nutoral]{
コードレビューが必須になるってことですか？
//}

//imagetalkl[mai_nutoral]{
そうとも言える。実際沢山のコードの変更を合流する最終チェックだからね、権限のある人が慎重にレビューしてプルリクエストをマージすることになる。
//}
...
//imagetalkl[mai_nutoral]{
あきなちゃんもそのうちこのプルリクエストでのコードレビューを任されるようになるからがんばってね。
//}
...
//imagetalkl[mai_nutoral]{
リモートリポジトリがコード管理システムのサーバーって説明をしたと思うけど、Gitを触り始めたばかりの人でも使えるリモートリポジトリにGitHubって言うのがある。
//}

//imagetalkr[akina_nutoral]{
あ、なんか聞いたことあります。なんか新しいソフトのコードを直接見れたような・・・。
//}

//imagetalkl[mai_nutoral]{
GitHubをリモートリポジトリに設定して公開設定をしておくと、誰でもそのリポジトリの内容を見れるようになるね。このような形でソフトウェアを配布している人も多いね。
//}
...
//imagetalkl[mai_nutoral]{
すごいよね。なんならコードの変更もプルリクエストを介して可能だ。この公開リポジトリの形式を活かしてオープンソースソフトウェア(OSS)活動をしているエンジニアも多いね。
//}
...
//imagetalkl[mai_niko]{
実際OSSでコードを沢山コミットしてる人はコーディング力がずば抜けてるからね。私たちも食らいつく感じでコミットしていけるといいね。
//}
...
//imagetalkl[mai_nutoral]{
問題って捉えてもいいけど議論が必要な議題って考えてもいいね。あとはチーム開発の開発項目にしても使える。Issueを作るとコミュニティのメンバーと議論しながらコードが必要かの結論が生まれて、コードを書かなきゃいけないって結論になると誰かが立候補するかして担当者がアサインされる。担当者によって開発された機能をプルリクエストにして管理者に送って内容に問題がなければプルリクエストをマージしてプロダクトが成長していくんだ。
//}

//imagetalkr[akina_nutoral]{
Issueを中心に開発が進んでいくんですね。
//}


//imagetalkl[mai_nutoral]{
そう！Issueのいいところはプルリクエストと紐づけることで、プルリクエストがマージされると同時に解決済みIssueとしてラベル付けされるんだ。これをIssueのクローズって呼ぶね。
//}

//imagetalkr[akina_hee]{
機能単位の開発がIssueの作成に始まってIssueのクローズによって終わるんですね。
//}

//imagetalkl[mai_niko]{
キレイだよね！さらに、このIssueと相性のいい考えが別の分野から発見されている。
//}
...
//imagetalkl[mai_nutoral]{
アジャイルのカンバンで管理するバックログは必要機能の深堀りによって細かい機能に分解することができる。それぞれの機能をIssueとして登録すると、開発者はIssueに関係するコードを開発してプルリクエストを作成して成果を可視化することができる。開発責任者はそのプルリクをコードレビューした後マージしてIssueを解決済みにしてクローズする。これだけで開発からレビューまでをプロセス化することができる。
//}

//imagetalkr[akina_nutoral]{
なるほど、バックログ・開発・レビューがキレイにつながるんですね。
//}

//imagetalkl[mai_nutoral]{
そのとおり！必要な機能とコードが１対１で紐づくから開発責任者もリリースに向けてどの機能が実装されているかの確認がとても鮮明になるからありがたい限りだね。
//}
...
//imagetalkl[mai_nutoral]{
	次は改めて、リモートブランチの重要なブランチをどう管理するかの話をしよう。まずは「main」などの中心となるブランチと「release」というリリース単位のブランチ、「dev」という開発用のブランチを使っていく。開発者は基本的に「dev」ブランチでコードを開発して実装できたら「release」ブランチにプルリクエストを出してコードを育てていく。開発責任者は計画に基づいたリリースタイミングで「main」ブランチへマージを行って実際のリリースを行う。このような設計を行うことをブランチ設計と呼ぶんだ。
//}

//imagetalkr[akina_nutoral]{
なるほど、ブランチごとの使い方から設計するんですね。今回3つのブランチ名を例に挙げてもらいましたけど、プロジェクトの現場に入るごとに名前が違うとかいうことは起きないんですか？
//}

//imagetalkl[mai_nutoral]{
ブランチ設計は推奨される設計標準としていくつかのパターンが公表されている。GitHubが出している「GitHubフロー」や「git-flow」が有名で、これらのようなブランチ設計を参考にするのが良いね。さっきの例はgit-flowに近いかな。重要なのはブランチのどこかに本番用のコードを置きつつ、変更を別ブランチから持ってくる形にするっていうことだ。
//}

//imagetalkr[akina_hee]{
なるほど、複数ブランチの運用を前提として、命名規則のようなものが標準化されてるんですね。
//}

//imagetalkl[mai_tohoho]{
これが実際ありがたい話でさ。毎回プロジェクトでブランチ名を議論するなんてそれこそ車輪の再発明だしね。もちろん、開発の規模やリリース計画によってリリースごとのブランチを切るかも変わってくることもあるけど。それらの成果物管理をブランチを使って行うっていうのがプロダクトの管理責任・開発責任に直結するから慎重な検討が必要だね。
//}

== ブランチ設計とテスト自動化

//imagetalkl[mai_nutoral]{
ブランチ設計の話で本番ブランチとそれ以外のブランチを作って運用する話をしたね。GitHubではプルリクエストの内容のコードのテストが成功するかどうかの自動テストや、コンパイル等を行う自動ビルドを行う設定を作ることができる。本番ブランチが別ブランチから変更を受ける度に自動テストを走らせるっていうのは、テスト自動化の時にも話したね。
//}
...
//imagetalkl[mai_nutoral]{
これによってビルド不可やテスト失敗を含むコードの本番環境へのマージを防ぐことができる。これは開発者にとっても開発責任者にとっても安心できる優れた構成で、この構成やこの状況を作る働きかけをContinueing Integration(CI)と呼ぶんだ。
//}

//imagetalkr[akina_hee]{
なんか聞いたことあります！・・・雲の上の話かと思ってましたが案外身近なんですね。
//}

//imagetalkl[mai_nutoral]{
構成を組み上げるのは結構骨が折れるけどね。それでも、その努力が骨折り損になることはまず無いね。労力を先行投資するにはうってつけの大事な作業だ。ひとつひとつ調べながら身に着けていこう！
//}
...
//imagetalkl[mai_nutoral]{
	さて自動ビルドや自動テストについて話したけど、もう一つ大切な要素がある。それは本番コードを実際に本番環境にデプロイする動きを自動化する「デプロイ自動化」だ。
//}
...
//imagetalkl[mai_niko]{
もちろん設定は一筋縄では行かないけど、コード管理システムから本番環境へのコード配置や、サービス再起動などのサーバー内挙動を管理したりすることでデプロイの自動化が可能になるんだ。ちなみにこのようなデプロイ自動化の構成や状況を作る働きかけをContinueing Delivery(CD)と呼ぶんだ。
//}

//imagetalkr[akina_hee]{
それも聞いたことあります。CIと並んでCI/CDって言われてますよね！
//}

//imagetalkl[mai_niko]{
そうそう！そしてさらにこのCI/CDとDockerやKubernetesなどの仮想化技術を組み合わせてインフラレベルでサービス提供の自動化を行う働きかけをDevOpsって呼んでるね。
//}
...
//imagetalkr[akina_ettu]{
長すぎるというか、世の中DevOpsって言葉をたくさん聞くようになってたことが、どれだけ進んだ話をしていたのか分かりました。
//}

//imagetalkl[mai_nutoral]{
ちなみにDevOpsエンジニアは業界単位で人材不足が叫ばれています。
//}
...
//imagetalkl[mai_nutoral]{
できる人が少ないから実践できてる現場も少ないってことだ。
//}

//imagetalkr[akina_ooo]{
なるほど・・・？それはまだチャンスがあるから良いこと・・・ではないか。
//}

//imagetalkl[mai_tohoho]{
ではないね。今までと何も変わらない地獄の現場が多いってことだ。
//}

//imagetalkr[akina_ugee]{
と、とりあえず勉強してみることにします、DevOps。
//}